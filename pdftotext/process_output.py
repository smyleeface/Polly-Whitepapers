...
def with_pdf(pdf_doc, pdf_pwd, fn, *args):
    ...
    try:
        ...
        # close the pdf file
        kargs = {}
        if fn.__name__ == '_parse_pages':
            kargs = {'fp': fp, 'images_folder': 'images'}
        result = fn(document, **kargs)
        fp.close()
    except IOError:
...
def _parse_pages(document, images_folder, fp):
    ...
    for i, page in enumerate(PDFPage.get_pages(fp, set(), 0, document)):
...
def parse_lt_objs(lt_objs, page_number, images_folder, text=[]):
    ...
    for lt_obj in lt_objs:
        ...
        #     else:
        #         print(f"error savxing image on page {page_number} {lt_obj.__repr__}")
...
if __name__ == "__main__":
    print('\n\n'.join(get_pages(f'{file_path}/{input_filename}', '', './images')))
    print(get_toc(f'{file_path}/{input_filename}', ''))
